...
function searchBtnHandler() {
    currentSearchQuery = document.querySelector('.search-field').value;
    document.querySelector('.autocomplete-list').innerHTML = '';
    document.querySelector('.autocomplete-list').classList.remove('has-items');
    downloadData(1);
}
    ...
    if (suggestions.length > 0) {
        suggestions.forEach(suggestion => {
            ...
            item.onclick = function() {
                ...
                autocompleteList.innerHTML = '';
                autocompleteList.classList.remove('has-items');
...
window.onload = function () {
    ...
    document.querySelector('.search-field').addEventListener('input', function() {
        autocompleteSuggestions(this.value);
    });
};